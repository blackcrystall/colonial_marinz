...
/obj/item/alien_embryo/proc/process_growth(delta_time)
	...
	if(affected_mob.in_stasis || affected_mob.bodytemperature < 170)
		if(stage < 5)
			counter += 0.33 * faction.larva_gestation_multiplier * delta_time
		else if(stage == 4)
			counter += 0.11 * faction.larva_gestation_multiplier * delta_time
		...
		counter += 1.5 * faction.larva_gestation_multiplier * delta_time //Currently twice as much, can be changed
	else
		if(stage < 5)
			counter += 1 * faction.larva_gestation_multiplier * delta_time
...
/obj/item/alien_embryo/proc/become_larva()
	...
	if(!picked)
		...
		var/list/candidates = get_alien_candidates(faction)
