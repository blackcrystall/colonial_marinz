...
/obj/effect/xenomorph/spray/Initialize(mapload, new_cause_data, datum/faction/faction_to_set) //Self-deletes
	. = ..()
	...
	for(var/atom/atm in loc)
		...
		if(istype(atm, /obj/flamer_fire))
			var/obj/flamer_fire/flamer_fire = atm
			if((flamer_fire.firelevel > fire_level_to_extinguish) && (!flamer_fire.fire_variant)) //If fire_variant = 0, default fire extinguish behavior.
				flamer_fire.firelevel -= fire_level_to_extinguish
				flamer_fire.update_flame()
			else
				switch(flamer_fire.fire_variant)
					if(FIRE_VARIANT_TYPE_B) //Armor Shredding Greenfire, extinguishes faster.
						if(flamer_fire.firelevel > 3*fire_level_to_extinguish)
							flamer_fire.firelevel -= 3*fire_level_to_extinguish
							flamer_fire.update_flame()
						else qdel(atm)
					else
						qdel(atm)
		...
		if(istype(atm, /obj/effect/alien/weeds))
			var/obj/effect/alien/weeds/weeds = atm

			if(!weeds.faction || weeds.faction != faction)
				weeds.acid_spray_act()
				continue
...
/obj/effect/xenomorph/spray/proc/apply_spray(mob/living/carbon/H, should_stun = TRUE)
	...
	if(!H.lying)
		...
		if(ishuman(H))
			...
			var/mob/living/carbon/xenomorph/xeno = H
			if(xeno.mob_size < MOB_SIZE_BIG && should_stun)
				xeno.KnockDown(stun_duration)
			xeno.emote("hiss")
...
/obj/effect/xenomorph/boiler_bombard/proc/damage_mobs()
	...
	for (var/mob/living/carbon/H in loc)
		if(isxeno(H))
			...
			var/mob/living/carbon/xenomorph/xeno = H
			if(source_xeno.can_not_harm(xeno))
				continue
...
/obj/effect/xenomorph/acid_damage_delay/New(loc, damage = 20, delay = 10, empowered = FALSE, message = null, mob/living/carbon/xenomorph/xeno = null)
	..(loc)
